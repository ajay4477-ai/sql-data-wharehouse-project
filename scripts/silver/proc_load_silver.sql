...
Script Purpose:
    ...
    populate the 'silver' schema tables from BEthe 'bronze' schema.
	Actions Performed:
...
BEGIN 
	...
	BEGIN TRY
		...
		cus_id,
		cus_key,
		TRIM(cus_firstname) AS cus_firstname,
		TRIM(cus_lastname) AS cus_lastname,

		CASE
			WHEN UPPER(TRIM(cus_marital_status)) = 'M' THEN 'Married'
			WHEN UPPER(TRIM(cus_marital_status)) = 'S' THEN 'Single'
			ELSE 'n/a'
		END AS cus_marital_status,

		CASE 
			WHEN UPPER(TRIM(cus_gndr)) = 'M' THEN 'Male'
			WHEN UPPER(TRIM(cus_gndr)) = 'F' THEN 'Female'
			ELSE 'n/a'
		END AS cus_gndr,
		...
		FROM (
			SELECT
				*,
				ROW_NUMBER() OVER(PARTITION BY cus_id ORDER BY cus_create_date) AS flag_last
